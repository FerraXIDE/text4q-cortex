...
class IBMQuantumConnector:
    ...
    def _ensure_connected(self) -> None:
        """Lazy connection — only connect when first job is submitted."""
    ...
    def execute(self, intent: CircuitIntent, qasm: str) -> CortexResult:
        """
        Compile the QASM circuit and execute it on the configured backend.

        Args:
            intent:  The parsed circuit intent (carries metadata like shots).
            qasm:    OpenQASM 3.0 circuit string.

        Returns:
            CortexResult with counts and execution metadata.
        """
